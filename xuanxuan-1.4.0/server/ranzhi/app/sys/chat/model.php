...
class chatModel extends model
{
    ...
    {
        $user = $this->dao->select('id, account, realname, avatar, role, dept, status, admin, gender, email, mobile, phone, site,reasons, qq, deleted')->from(TABLE_USER)->where('id')->eq($userID)->fetch();
        if($user)
        {
            $user = $this->formatUsers($user);
        }
    ...
    {
        $dao = $this->dao->select('id, account, realname, avatar, role, dept, status, admin, gender, email, mobile, phone, site,reasons, qq, deleted')
